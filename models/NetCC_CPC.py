"""
CC-CPC network: confusion-aware boundary-enhanced prototype contrastive learning
Based on NetMultiClass, replace the standard CPC with ConfusionBoundaryCPC
Also pass in edge_map to support boundary-adaptive temperature
"""

import torch
import torch.nn as nn
import torch.nn.functional as F
from models.model import (Conv, UpSampler, FAI, DWT, BasicBlockL, BasicBlockH,
                           CIU, ConfusionBoundaryCPC)
from models.smt import smt_t


class NetCC_CPC(nn.Module):
    """
    CC-CPC: Confusion-aware & Contrast-enhanced Prototype Contrast
    On top of the standard CPC, add:
    1. confusion-aware weighting - dynamically track class confusion and strengthen contrastive learning for hard class pairs
    2. boundary-enhanced contrast - use a sharper temperature at target boundaries to improve discrimination
    """
    def __init__(self, num_classes=5, temperature=0.07, enable_refinement=True,
        ...
        # ===== CC-CPC module =====
        self.cpc = ConfusionBoundaryCPC(
            in_channels=self.channels[1],
            num_classes=num_classes,
            proto_dim=64,
            temperature=self.temperature,
            enable_refinement=self.enable_refinement,
            confusion_momentum=confusion_momentum,
    ...
    def forward(self, RGB, gt_labels=None, edge_map=None):
        """
        Args:
            RGB: input image (B, 3, H, W)
            gt_labels: ground-truth labels (B, H, W) int, used during training
            edge_map: edge GT (B, H, W), used during training
        """
        ...
        # CC-CPC: pass in gt_labels and edge_map
        cpc_loss = None
        F1, cpc_loss = self.cpc(F1, gt_labels, edge_map)
    ...
    def load_pre(self, pre_model):
        ...
        print(f"NetCC_CPC loading pre_model: {pre_model}")

    def load_from_multiclass_weights(self, weight_path):
        """
        Load from standard-CPC NetMultiClass weights
        Copy all weights except the CPC module; the CC-CPC module is randomly initialized
        """
        print('Loading from NetMultiClass weights:', weight_path)
        ...
        for k, v in state_dict.items():
            if 'Sal_Head' in k:
                skipped_count += 1
                continue
            # skip the CPC module weights (CC-CPC has extra buffers and a different structure)
            if 'cpc' in k:
                skipped_count += 1
                continue
        ...
        print('CC-CPC module randomly initialized.')
